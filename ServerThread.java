...
public class ServerThread extends Thread
{
    ...
    public void getKey() {
      ...
      // now lets generate a safe prime
      debug("Generating p");
      boolean not_found = true;
      do {
        q = BigInteger.probablePrime(prime_bit_len, rand); // get q
        p = two.multiply(q).add(BigInteger.ONE);  // let p = 2q+1
        // check if p is prime
      ...
      // now lets generate a g primitive root of p
      debug("Generating g primitive root of p");
      g =  new BigInteger("2");
      ...
      BigInteger temp = g;
      do {
        if (!(temp.modPow(q,p).compareTo(BigInteger.ONE) == 0))
        {
          g = temp;
          not_found = false;
        }
      ...
      BigInteger a;
      do {
          a = new BigInteger(phi.subtract(BigInteger.ONE).bitLength(), rand);
          debug("a = " + a );
      } while (a.compareTo(phi.subtract(BigInteger.ONE)) >= 0);

      // calculate g^a
      dkey = g.modPow(a,p);
      // send the partial key to client
      debug("Sending g^a to client");
      try {
      ...
      // receive the partial key form the client
      debug("Recieving 'g^b'");
      BigInteger cp_key = new BigInteger("0");
      try {
        byte[] cp_key_a = CryptoUtilities.receive(in);
  		  cp_key = new BigInteger(cp_key_a);
      } catch (IOException e) {
        System.out.println(e);
      }

		  debug("Generating 'g^ab'");
		  dkey = cp_key.modPow(a,p);
